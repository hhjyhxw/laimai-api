package com.icloud.api.sevice.category;
...
import com.icloud.modules.lm.dto.CategoryTreeNodeDTO;
...
public class CategoryBizService {
    ...
    /*获取一棵两级目录树*/
    public List<CategoryTreeNodeDTO> categorySecondLevelTree() throws ApiException {
        //获取所有一级分类 数据库存储level级别 1 对应页面节点数level 0
        List<LmCategory> categoryDOS = lmCategoryMapper.selectList(new QueryWrapper<LmCategory>().le("level",1 ).orderByDesc("level"));
        List<CategoryTreeNodeDTO> list = categoryDOS.stream().filter((item) -> (item.getParentId().equals(0))).map(item -> {
            CategoryTreeNodeDTO dto = new CategoryTreeNodeDTO();
            dto.setLabel(item.getTitle());//分类名称
            dto.setLevel(0);//
    ...
    public List<CategoryDTO> categoryList() throws ServiceException {
        ...
        //从数据库查询
        List<LmCategory> categoryDOList = lmCategoryMapper.selectList(new QueryWrapper<>());
        //组装DTO
